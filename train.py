...
def build_optimizer(loss):
  #set up, grab every single trainable weights and bias from both NN
  optimizer = tf.train.AdamOptimizer(learning_rate=config.LEARNING_RATE)
  trainable_vars = tf.traninable_variables()

  # creates a shadow copy of the NN where every weight is 0.0, these are buckets
  accumulators = [tf.Variable(tf.zeros_like(tv.initialized_value()), trainable=False) for tv in trainable_vars]
  accumulation_counter = tf.Variable(0.0, trainable=False)

  #calculates hte math for backprop, but doesnt update the weights yet.
  grad_pairs = optimizer.compute_gradients(loss, trainable_vars)

  #pours math into bucket
  ...
  #because 2 distinct NN, safety CBF and action network controller, might want to train on different schedules
  #seperate gradients
  gradient_vars = [(accumulator / accumulation_counter, var) for (accumulator, (grad, var)) in zip(accumulators, grad_pairs)]

  #first calculate the true average gradient by divding the buckets content by hte number of micro batches
  #it looks at string name of every single weight in the system.
  gradient_vars_h = []
  gradient_vars_a = []
  ...
  #the optimizer takes the averaged gradients and permanently updates the weight of the safety network  
  train_step_h = optimizer.apply_gradients(gradient_vars_h)
  train_step_a = optimizer.apply_gradients(gradient_vars_a)

  #clean up
  zero_ops = [accumulators.assign(tf.zeros_like(tv) for (accumulator, tv) in zip(accumulators, trainable_vars))]
...
def build_training_graph(num_agents):
  #entry points
  #s is the state vector of the agents
  s = tf.placeholder(tf.float32, [num_agents, 4])
  #g is the goal states
  g = tf.placeholder(tf.float32, [num_agents, 2])
  #x is the difference between the state of each agent with other agents
  x = tf.expand_dims(s, 1) - tf.expand_dims(s, 0)

  #agent looks at the worl, evaluate safety, then steer
  #h is the cbf value of shape [num_agent, top_k, 1]
  h, mask, indices = core.network_cbf(x=x, r=config.DISTANCE_MIN_THRESHOLD, indices=None)
  #a is the control action of each agent, with shape [num_agents, 2]
  a = core.network_action(s=s, g=g, obervsation_radius=config.OBSERVATION_RADIUS, indices=indices)

  #compute the value of loss functions and the accuracies
  #loss_dang is for h(s) < 0, s in dangerous set
  #loss_safe is for h(s) >=0, s in the safe set
  #acc_dang is the accuracy that h(s) < 0, s in dangerous set is satified
  #acc_safe is the accuracy that h(s) >=0, s in the safeset
  loss_dang, loss_safe, acc_dang, acc_safe = core.loss_barrier(h=h, s=s, r=config.DISTANCE_MIN_THRESHOLD, ttc=config.TIME_TO_COLLISION, indices=indices)

  #loss dan deriv is for doth(s) + alpha(s) >=0 in dangerous set. 
  #loss safe deriv is for dot h(s) + alpha(s) >=0 in the safe set
  loss_dang_deriv, loss_safe_deriv, acc_dang_deriv, acc_safe_deriv = core.loss_derivatives(s=s, a=a, h=h, x=x, r=config.DISTANCE_MIN_THRESHOLD, indices=indices, ttc=config.TIME_TO_COLLISION, alpha=config.ALPHA_CBF)

  #distance between a and the nominal a
  loss_action = core.loss_action(s=s, g=g, a=a, r=config.DISTANCE_MIN_THRESHOLD, ttc=config.TIME_TO_COLLISION)
